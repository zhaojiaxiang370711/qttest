...
### GAP-9: `lint` Does Not Resolve Built QML Modules ✅ CLOSED

> **Status (2026-07-15): CLOSED** — `qtcli lint` now discovers import roots under `build/` (including `build/.qt/qml` and roots derived from generated `qmldir` module URIs) and passes each one to `qmllint` with `-I`. Verified after migrating this project to the `QxznHmi` executable module: **6 files, 0 with diagnostics**. Running `qmllint` directly without `-I build` still fails to import `QxznHmi` and reports the singleton references as unqualified.

**Problem:** `qtcli lint` ran `qmllint <file>` independently for every source file. C++ types generated by `qt_add_qml_module()` are described by build-tree `qmldir` / `.qmltypes` files, so source-only linting could not resolve project modules such as `QxznHmi`, `Config`, or `SessionModel`.

**Enhancement:** Make `lint`:
- Discover conventional and generated QML import roots in the project build tree
- Derive custom roots from `qmldir` entries such as `module Acme.Controls`
- Invoke `qmllint -I <root> ... <file>` while preserving standalone behavior when no build directory exists

**Impact:** High — typed QML singletons and other project-local module types now lint correctly instead of being mistaken for unqualified context properties.
...
| Project discovery (find-projects) | Correct and filtered |
| QML awareness | Covered — GAP-1 through GAP-9 closed |

**qtcli v0.1 now covers both the C++/CMake inspection layer and the core QML project workflow surfaced by this dogfood port.** All 9 recorded gaps are closed, including build-aware linting for generated QML modules.
